<%@page import="cn.com.oking.em.entity.EmHospital"%>
...
<%
  	List<EmHospital> data=(List<EmHospital>)session.getAttribute("maps");
 	for (int i = 0; i < data.size(); i ++) {  
 		EmHospital em=data.get(i);
  		if(i==0&&data.size()==1){
	%> //设置中心坐标以及缩放程度
	mapObj.setZoomAndCenter(16, new AMap.LngLat(<%=em.getLongitude()%>,<%=em.getLatitude()%>));
      ...
      AMap.event.addListener(marker, 'click', function (entity) {  
    	  ...
    	  var infoWindow = new AMap.InfoWindow({  
            	isCustom:true,
                content:createInfoWindow("详细信息","场  所：<%=em.getPlacename()%><br/>位  置：<%=em.getLocation()%> <br/>负责人：<%=em.getContraces()%> <br/>联系电话：<%=em.getPhone()%> <br/>"), 
