...
public class ExcelServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			NkRecordAction action=ContextHolder.getBean("nkRecordAction");
			NkRecord nkrecord=action.getDataById(id);
			//查询上传记录表
			FileInformationAction sAction=ContextHolder.getBean("fileInformationAction");
			sysfileinformation = sAction.getInformationByAss(nkrecord.getFileid());
		...
		//从request中取出要下载文件的名字  
		 ...
		 if(contentType==null)  
			...
			e1.printStackTrace();
			response.getWriter().println("<script='javascript>javascript:history.back()</script>'");
		}
	}
}
