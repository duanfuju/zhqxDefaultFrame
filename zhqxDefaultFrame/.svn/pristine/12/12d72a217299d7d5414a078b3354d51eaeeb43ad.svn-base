<%@page import="cn.com.oking.em.entity.XEmSupplies"%>
...
function initialize(){
  ...
  var markers = []; 
<%
  	List<XEmSupplies> data=(List<XEmSupplies>)session.getAttribute("maps");
 	for (int i = 0; i < data.size(); i ++) {  
  		XEmSupplies em=data.get(i);
  		if(i==0&&data.size()==1){
	%> //设置中心坐标以及缩放程度
	mapObj.setZoomAndCenter(12, new AMap.LngLat(<%=em.getLongitude()%>,<%=em.getLatitude()%>));
		<%}%>
      ...
      AMap.event.addListener(marker, 'click', function (entity) {  
          var infoWindow = new AMap.InfoWindow({  
          	isCustom:true,
              content:createInfoWindow("详细信息","<%=em.getDescription()%><%=em.getQuantity()%><%=em.getUnit()%><br/>位  置：<%=em.getLocation()%> <br/>负责人：<%=em.getContraces()%> <br/>联系电话：<%=em.getPhone()%> <br/>"), 
              size:new AMap.Size(300, 0),  
              autoMove:true,   
              offset:new AMap.Pixel(10,-30)  
          });                
          infoWindow.open(mapObj, new AMap.LngLat(entity.lnglat.lng,entity.lnglat.lat));  
          mapObj.setZoomAndCenter(12, new AMap.LngLat(entity.lnglat.lng,entity.lnglat.lat));
      });  
      markers.push(marker);  
<%
     }
